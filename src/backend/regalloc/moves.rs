...
fn plan_entry_moves(
    ...
    for param in entry.params.iter() {
        ...
        let alloc = alloc_map
            .get(&param_id)
            .copied()
            .unwrap_or_else(|| panic!("backend regalloc: missing alloc for {:?}", param_id));
        ...
        match pass.kind {
            PassKind::Reg => {
                let src = src_locs
                    .first()
                    .copied()
                    .unwrap_or_else(|| panic!("backend regalloc: missing ABI src for param"));
                if src != alloc {
            ...
            PassKind::ByValueAgg => {
                let Location::Stack(slot) = alloc else {
                    panic!(
                        "backend regalloc: aggregate param must be stack-backed, got {:?}",
                        alloc
                    );
                };
...
fn edge_moves_for(
    ...
    let params = block_params.get(&to).unwrap_or_else(|| {
        panic!(
            "backend regalloc: missing params for target block {:?} from {:?}",
            to, from
        )
    });
    ...
    if params.len() != args.len() {
        panic!(
            "backend regalloc: block {:?} expects {} args, got {}",
            to,
            params.len(),
            args.len()
        );
    ...
    for (arg, param) in args.iter().zip(params.iter()) {
        let src = alloc_map
            .get(arg)
            .copied()
            .unwrap_or_else(|| panic!("backend regalloc: missing alloc for {:?}", arg));
        let dst = alloc_map
            .get(param)
            .copied()
            .unwrap_or_else(|| panic!("backend regalloc: missing alloc for {:?}", param));
        if src != dst {
            let arg_ty = value_types
                ...
                .unwrap_or_else(|| {
                    panic!(
                        "backend regalloc: missing type for block arg {:?} (param {:?})",
                        arg, param
                    )
                });
...
pub(crate) fn abi_arg_locations(
    ...
    if pass.kind != PassKind::ByValueAgg || pass.size <= 8 {
        if *next_reg < param_reg_count {
            let reg = target
                ...
                .unwrap_or_else(|| panic!("backend regalloc: arg {} has no ABI reg", *next_reg));
            *next_reg += 1;
    ...
    if *next_reg + 1 < param_reg_count {
        ...
        let reg1 = target
            ...
            .unwrap_or_else(|| panic!("backend regalloc: arg {} has no ABI reg", *next_reg + 1));
        *next_reg += 2;
...
fn call_arg_type(
    ...
    value_types
        .get(arg)
        .copied()
        .unwrap_or_else(|| panic!("backend regalloc: missing type for arg {:?}", arg))
}
        ...
        match pass.kind {
            PassKind::Reg => {
                ...
                let dst = dst_locs
                    .first()
                    .copied()
                    .unwrap_or_else(|| panic!("backend regalloc: missing ABI dst for arg"));
                if src != dst {
                    let size = move_size_for(types, arg_ty, src, dst);
                    pre_moves.push(MoveOp { src, dst, size });
                }
            ...
            PassKind::ByRef => {
                let src = match call_arg_src(*arg, alloc_map) {
                    ...
                    other => {
                        panic!(
                            "backend regalloc: aggregate arg must be stack-backed, got {:?}",
                            other
                        );
                    }
                };
            ...
            PassKind::ByValueAgg => {
                let src_slot = match call_arg_src(*arg, alloc_map) {
                    ...
                    other => {
                        panic!(
                            "backend regalloc: by-value aggregate arg must be stack-backed, got {:?}",
                            other
                        );
                    }
                };

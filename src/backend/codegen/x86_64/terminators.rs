...
impl X86_64Emitter {
    ...
    pub(super) fn emit_cond_branch_impl(
        ...
        let cond_reg = self.load_value_typed(
            locs,
            cond_loc,
            cond_ty,
            crate::backend::regalloc::target::PhysReg(10),
        );
    ...
    pub(super) fn emit_switch_impl(
        ...
        let value_reg = self.load_value_typed(
            locs,
            value_loc,
            value_ty,
            crate::backend::regalloc::target::PhysReg(10),
        ...
        for (case, target) in cases {
            self.emit_mov_imm("%r11", case.as_int(), 64);
            self.emit_line(&format!("cmpq %r11, {}", value_reg));
    ...
    pub(super) fn emit_terminator_impl(&mut self, term: &Terminator, locs: &LocationResolver) {
        match term {
            ...
            Terminator::Return { value } => {
                if let Some(value) = value {
                    ...
                    if needs_sret(locs, ty) {
                        let sret_reg = crate::backend::regalloc::target::PhysReg(5);
                        let offset = self
                        ...
                        let src_reg = self.load_value_typed(
                            locs,
                            src,
                            ty,
                            crate::backend::regalloc::target::PhysReg(10),
                        ...
                        match size {
                            ...
                            1 | 2 | 4 => self.emit_line(&format!("movl {}, %eax", src_reg)),
                            8 => self.emit_line(&format!("movq {}, %rax", src_reg)),
                        ...
                        if size == 0 {
                            ...
                            self.emit_load_sized(
                                crate::backend::regalloc::target::PhysReg(0),
                                "rsp",
                                base,
                                size.min(8),
                            );
                            if size > 8 {
                                self.emit_load_sized(
                                    crate::backend::regalloc::target::PhysReg(2),
                                    "rsp",
            ...
            Terminator::Unreachable => {
                self.emit_line(&format!("call _{}", RuntimeFn::Trap.name()));
                self.emit_line("ud2");
            }
        }
    }

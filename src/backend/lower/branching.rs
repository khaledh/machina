...
use crate::backend::lower::join::JoinSession;
use crate::backend::lower::locals::LocalValue;
use crate::backend::lower::lowerer::{
...
use crate::ir::{BlockId, Callee, CastKind, CmpOp, ConstValue, SwitchCase, Terminator, ValueId};
...
impl<'a, 'g> FuncLowerer<'a, 'g> {
    ...
    pub(super) fn lower_try_handle(
        ...
        let operand_sem_ty = self
            .type_map
            .type_table()
            .get(self.type_map.type_of(fallible_expr.id))
            .clone();

        if matches!(handler_expr.kind, ExprKind::Block { .. }) {
            if let Some(ok_ty) = operand_sem_ty.nullable_address_payload() {
    ...
    fn lower_try_handle_inline_nullable(
        &mut self,
        expr: &Expr,
        fallible_expr: &Expr,
        handler_expr: &Expr,
        ...
        let join_expr_ty = self.type_map.type_of(expr.id);

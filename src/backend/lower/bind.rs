...
impl<'a, 'g> FuncLowerer<'a, 'g> {
    ...
    fn bind_array_pattern(
        ...
        for (suffix_index, pattern) in suffix.iter().enumerate() {
            ...
            let index_val = self
                .builder
                .binop(crate::ir::BinOp::Sub, view.len, offset_val, u64_ty);
            let elem_addr = self.builder.index_addr(view.ptr, index_val, elem_ptr_ty);
            let elem_val = self.builder.load(elem_addr, elem_ir_ty);
            self.bind_pattern(pattern, LocalValue::value(elem_val, elem_ir_ty), &elem_ty)?;
        }
            ...
            && let Some(rest_pattern) = &rest.pattern
        {
            ...
            let end_val = self
                .builder
                .binop(crate::ir::BinOp::Sub, view.len, suffix_len, u64_ty);
    ...
    fn emit_array_pattern_min_len_check(&mut self, len: crate::ir::ValueId, min_len: usize) {
        if min_len == 0 {
            return;
        }
        let u64_ty = self.type_lowerer.lower_type(&Type::uint(64));
    ...
    fn emit_array_pattern_exact_len_check(&mut self, len: crate::ir::ValueId, exact_len: usize) {
        let u64_ty = self.type_lowerer.lower_type(&Type::uint(64));

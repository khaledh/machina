...
impl<'a, 'b, 'g> MatchLowerer<'a, 'b, 'g> {
    ...
    fn lower_decision_tree(
        ...
        if arms.len() != arm_plans.len() {
            panic!(
                "backend match plan arm mismatch: {} arms vs {} plans",
                arms.len(),
                arm_plans.len()
            );
        }
    ...
    fn lower_discriminant(
        ...
        if let Type::Enum { .. } = ty {
            let enum_ty_id = self
                ...
                .type_table()
                .lookup_id(&ty)
                .unwrap_or_else(|| panic!("backend match missing enum type id for {:?}", ty));
            let tag_ty = {
                ...
                sem::MatchProjection::Field { index } => match &curr_ty {
                    ...
                    Type::Enum { .. } => {
                        let enum_ty_id = self
                            ...
                            .lookup_id(&curr_ty)
                            .unwrap_or_else(|| {
                                panic!("backend match missing enum type id for {:?}", curr_ty)
                            });
                        let (tag_ty, blob_ty) = {
                            let layout = self.lowerer.type_lowerer.enum_layout(enum_ty_id);
                        ...
                        let field_ir_ty = match *index {
                            0 => tag_ty,
                            1 => blob_ty,
                            _ => panic!("backend match enum field out of range {index}"),
                        };
                        addr = self.lowerer.field_addr_typed(addr, *index, field_ir_ty);
                        curr_ty = match *index {
                            0 => Type::Int {
    ...
    fn case_const(&self, value: u64, discr_ty: &Type) -> ConstValue {
        match discr_ty {
            ...
            other => panic!("backend match switch on unsupported type {:?}", other),
        }
    }
}

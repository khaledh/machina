//! Native support artifact helpers for batch build/run paths.
//!
//! Phase 1 of the module-interface plan keeps native support separate from user
//! compilation. The runtime C sources are compiled once into a cached static
//! archive and linked into each built executable. Prelude support code follows
//! the same pattern: `std/prelude_impl.mc` is compiled once into a cached
//! object file and reused across build/run paths. This avoids recompiling the
//! support layer on every user build while keeping the current linker model
//! intact.
...
fn compile_c_object(source: &Path, object: &Path) -> Result<(), String> {
...
fn archive_objects(archive: &Path, objects: &[PathBuf]) -> Result<(), String> {
    if archive.exists() {
...
fn artifact_is_stale(artifact: &Path, sources: &[PathBuf]) -> Result<bool, String> {
...
fn native_support_dir() -> Result<PathBuf, String> {

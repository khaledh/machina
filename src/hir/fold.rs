...
pub trait AstFolder {
    ...
    fn visit_func_def(&mut self, func_def: &FuncDef) -> Result<Self::Output, Self::Error> {
    ...
    fn visit_match_arms<U>(
        &mut self,
        arms: &[MatchArm],
        visit_arm: impl FnMut(&mut Self, &MatchArm) -> Result<U, Self::Error>,
...
pub fn walk_func_def<F: AstFolder + ?Sized>(
    f: &mut F,
    func_def: &FuncDef,
...
pub fn walk_match_arms<F: AstFolder + ?Sized, U>(
    f: &mut F,
    arms: &[MatchArm],
    mut visit_arm: impl FnMut(&mut F, &MatchArm) -> Result<U, F::Error>,

...
use crate::mcir::types::{
    BlockId, Body, LocalId, LocalKind, PlaceAny, Rvalue, Statement, TyId, TyKind, TyTable,
};
use crate::mcregalloc::constraints::{CallConstraint, ConstraintMap, FnParamConstraint};
use crate::mcregalloc::liveness::{
    LiveInterval, LiveIntervalMap, LivenessAnalysis, build_live_intervals,
};
use crate::mcregalloc::moves::{FnMoveList, Location};
use crate::mcregalloc::pos::{InstPos, RelInstPos};
use crate::mcregalloc::{AllocationResult, LocalAllocMap, LocalClass, MappedLocal};
...
fn scan_address_taken(body: &Body) -> HashSet<LocalId> {
...
pub(crate) fn classify_locals(body: &Body) -> Vec<LocalClass> {
...
impl<'a> RegAlloc<'a> {
    pub fn new(body: &'a Body, constraints: &'a ConstraintMap) -> Self {
        Self {
            body,
            constraints,
    ...
    fn build_pos_map(body: &Body) -> PosIndexMap {

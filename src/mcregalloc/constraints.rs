...
use crate::mcir::types::{BlockId, Body, LocalId, LocalKind, PlaceAny, Statement, Terminator};
...
pub fn analyze_fn_params(body: &Body) -> Vec<FnParamConstraint> {
    let mut out = Vec::new();
...
pub fn analyze_fn_return(body: &Body) -> HashMap<BlockId, FnReturnConstraint> {
    let ret_local = body.ret_local;
    let ret_ty = body.locals[ret_local.index()].ty;
    if body.types.get(ret_ty).is_aggregate() {
...
pub fn analyze_calls(body: &Body) -> Vec<CallConstraint> {
...
pub fn analyze_constraints(body: &Body) -> ConstraintMap {
    ConstraintMap {

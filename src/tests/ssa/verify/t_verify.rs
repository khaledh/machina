...
use crate::ssa::lower::lower_module;
use crate::ssa::model::ir::{BinOp, InstKind, Instruction, Terminator, ValueDef, ValueId};
use crate::ssa::verify::verify_module;
...
fn lower(source: &str) -> crate::ssa::lower::LoweredModule {
...
"#;
    ...
    let non_ptr_value = func
        ...
        .find(|value| !matches!(types.kind(value.ty), crate::ssa::IrTypeKind::Ptr { .. }))
        .map(|value| value.id);
    let next_id = func
        .blocks
        .iter()
    ...
    let non_ptr_local = func
        .locals
        .iter()
        .find(|local| !matches!(types.kind(local.ty), crate::ssa::IrTypeKind::Ptr { .. }))
        .map(|local| local.ty);
    ...
    for block in &mut func.blocks {
        ...
        if let Some(local_ty) = non_ptr_local {
            ...
            block.insts.push(Instruction {
                ...
                kind: InstKind::Const {
                    value: crate::ssa::model::ir::ConstValue::Int {
                        value: 0,
                        signed: false,
                        bits: 64,
                    },

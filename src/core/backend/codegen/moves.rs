...
use crate::core::backend::regalloc::moves::{CallMove, EdgeMove, MoveOp, ParamCopy};
use crate::core::ir::{BlockId, Function};
...
fn validate_moves(context: &str, moves: &[MoveOp]) {
    ...
    {
        panic!(
            "backend codegen: illegal move in {} at {}: {:?} -> {:?}",
...
fn is_legal_move(src: Location, dst: Location) -> bool {
    matches!(
        ...
        (Location::Reg(_), Location::Reg(_))
            ...
            | (Location::Reg(_), Location::StackOffset(_, _))
            | (Location::Stack(_), Location::Reg(_))
            | (Location::StackOffset(_, _), Location::Reg(_))

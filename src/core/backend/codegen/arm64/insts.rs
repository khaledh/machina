...
impl Arm64Emitter {
    pub(super) fn emit_inst_impl(&mut self, inst: &Instruction, locs: &LocationResolver) {
        match &inst.kind {
            InstKind::Const { value } => {
                self.emit_const(inst, locs, value);
            }
            InstKind::BinOp { op, lhs, rhs } => {
                self.emit_binop(inst, locs, op, *lhs, *rhs);
            }
            ...
            InstKind::FieldAddr { base, index } => {
                self.emit_field_addr(inst, locs, *base, *index);
            }
            InstKind::IndexAddr { base, index } => {
                self.emit_index_addr(inst, locs, *base, *index);
            }
            InstKind::Store { ptr, value } => {
                self.emit_store(locs, *ptr, *value);
            }
        ...
        if let Some(result) = &inst.result {
            ...
            match value {
                ...
                ConstValue::FuncAddr { def } => {
                    ...
                    self.emit_line(&format!("add {}, {}, {}@PAGEOFF", dst_reg, dst_reg, label));
                    self.store_if_needed_typed(locs, dst_slot, &dst_reg, dst_ty);
                }
    ...
    fn emit_binop(
        ...
        if let Some(result) = &inst.result {
            ...
            let lhs_reg = self.load_value_typed(locs, lhs_loc, lhs_ty, "x9");
            ...
            let rhs_reg = self.load_value_typed(locs, rhs_loc, rhs_ty, rhs_scratch);
            let dst = locs.value(result.id);
            let dst_ty = locs.value_ty(result.id);
            let (dst_reg, dst_slot) = self.value_dst_typed(locs, dst, "x11", "binop", dst_ty);
            match op {
                BinOp::Mod => {
                    ...
                    self.emit_line(&format!("{} {}, {}, {}", div_op, q_reg, lhs_reg, rhs_reg));
                    // dst = lhs - q * rhs
    ...
    fn emit_int_trunc(
        ...
        if let Some(result) = &inst.result {
            ...
            let scratch = if src_reg == "x10" { "x11" } else { "x10" };
            let dst_ty = locs.value_ty(result.id);
            let (dst_reg, dst_slot) = self.value_dst_typed(locs, dst, scratch, "trunc", dst_ty);
            ...
            match dst_bits {
                ...
                16 => self.emit_line(&format!("and {}, {}, #0x{:x}", dst_reg, src_reg, 0xffffu64)),
                8 => self.emit_line(&format!("and {}, {}, #0x{:x}", dst_reg, src_reg, 0xffu64)),
                other => {
    ...
    fn emit_int_extend(
        ...
        if let Some(result) = &inst.result {
            ...
            let scratch = if src_reg == "x10" { "x11" } else { "x10" };
            let dst_ty = locs.value_ty(result.id);
            let (dst_reg, dst_slot) = self.value_dst_typed(locs, dst, scratch, "extend", dst_ty);
            let src_reg = Self::x_reg(&src_reg);
            let dst_reg = Self::x_reg(&dst_reg);
    ...
    fn emit_field_addr(
        ...
        if let Some(result) = &inst.result {
            ...
            let dst_ty = locs.value_ty(result.id);
            let (dst_reg, dst_slot) = self.value_dst_typed(locs, dst, "x9", "field-addr", dst_ty);
    ...
    fn emit_index_addr(
        ...
        if let Some(result) = &inst.result {
            ...
            let dst_ty = locs.value_ty(result.id);
            let (dst_reg, dst_slot) = self.value_dst_typed(locs, dst, "x11", "index-addr", dst_ty);
            ...
            if stride == 0 {
                ...
                if shift == 0 {
                    // Stride 1: plain add.
                    self.emit_line(&format!("add {}, {}, {}", dst_reg, base_reg, index_reg));
                } else {
    ...
    fn emit_store(&mut self, locs: &LocationResolver, ptr_id: ValueId, value_id: ValueId) {
        ...
        // Fast path: fold `field_addr` or `index_addr` to store directly at base+offset.

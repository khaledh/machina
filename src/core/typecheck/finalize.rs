...
fn build_outputs(engine: &TypecheckEngine) -> FinalizeOutput {
    ...
    record_linear_machine_lookup_call_sigs(engine, &mut builder);
...
fn record_linear_machine_lookup_call_sigs(engine: &TypecheckEngine, builder: &mut TypeMapBuilder) {
    for obligation in &engine.state().constrain.expr_obligations {
        let ExprObligation::LinearMachineLookup {
            expr_id,
            receiver,
            type_name,
            key_term,
